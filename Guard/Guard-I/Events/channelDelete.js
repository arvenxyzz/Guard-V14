const { isOwner, isProtectedGuild, banUser, checkWhitelist, useWhitelistLimit, isShieldEnabled } = require('../../Shared/Utils');
const { sendLog } = require('../../Shared/Logger');
const { getChannelBackup, restoreChannel } = require('../../Shared/Backup');

module.exports = {
    name: 'channelDelete',
    async execute(channel, client) {
        if (!channel.guild) return;
        if (!isProtectedGuild(channel.guild.id)) return;
        if (!await isShieldEnabled(channel.guild.id, 'channelShield')) return;

        const fetchedLogs = await channel.guild.fetchAuditLogs({ limit: 1, type: 12 });
        const log = fetchedLogs.entries.first();
        if (!log) return;

        const { executor } = log;
        if (executor.id === client.user.id) return;

        if (isOwner(executor.id)) {
            await sendLog(client, {
                title: '🛡️ Guard-I | Kanal Silme',
                executorTag: executor.tag,
                executorID: executor.id,
                action: 'Kanal Silme',
                target: `${channel.name} (${channel.id})`,
                targetID: channel.id,
                logType: 'channel',
                isOwner: true
            });
            return;
        }

        const member = await channel.guild.members.fetch(executor.id).catch(() => null);
        if (member) {
            const wl = await checkWhitelist(channel.guild.id, member, 'channel');
            if (wl.whitelisted) {
                const remaining = await useWhitelistLimit(wl.doc);
                await sendLog(client, {
                    title: '🛡️ Guard-I | Kanal Silme',
                    executorTag: executor.tag,
                    executorID: executor.id,
                    action: 'Kanal Silme',
                    target: `${channel.name} (${channel.id})`,
                    targetID: channel.id,
                    logType: 'channel',
                    isWhitelisted: true,
                    remainingLimit: remaining
                });
                return;
            }
        }

        const reason = 'Guard - Kanal Silme';
        const banned = await banUser(channel.guild, executor.id, reason);

        const channelBackup = await getChannelBackup(channel.guild.id, channel.id);
        let restored = false;
        if (channelBackup) {
            const newChannel = await restoreChannel(channel.guild, channelBackup);
            if (newChannel) restored = true;
        }

        await sendLog(client, {
            title: '🛡️ Guard-I | Kanal Silme',
            executorTag: executor.tag,
            executorID: executor.id,
            action: 'Kanal Silme',
            target: `${channel.name} (${channel.id})`,
            targetID: channel.id,
            logType: 'channel',
            punishment: banned ? 'Sunucudan Yasaklandı' : 'Ban başarısız',
            reason: reason,
            restored: restored
        });
    }
};
